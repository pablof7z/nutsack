...
function HistoryItem({ wallet, item, index, target }: { wallet: NDKWallet, item: NDKEvent, index: number, target: any }) {
...
function History({ wallet }: { wallet: NDKCashuWallet }) {
    ...
    return (
        <List
            ...
            sectionHeaderAsGap
            variant="insets"
            renderItem={({ item, index, target }) => <HistoryItem wallet={wallet} item={item} index={index} target={target} />}
        />
    )
}
                ...
                options={{
                    ...
                    headerRight: () => (
                        <TouchableOpacity onPress={() => router.push('/(settings)')}>
                            <Cog size={24} color={colors.foreground} />
                        </TouchableOpacity>
                    ),
                }}
            />
